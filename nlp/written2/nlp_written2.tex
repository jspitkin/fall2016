...
\part{d} $P(girl \ | \ young) = \frac{freq(W_{n-1}, W_n)}{freq(W_{n-1})} = \frac{freq(young \ girl)}{freq(young)} = \ $\framebox[2\width]{$\cfrac{2}{2}$}
...
\part{j} $P(thanked \ | \ VERB) = \frac{C(thanked \ with \ tag \ VERB)}{C(any \ word \ with \ tag \ T_i)} = \frac{C(thanked \ with \ tag \ VERB)}{C(any \ word \ with \ tag \ VERB)} = \ $\framebox[2\width]{$\cfrac{2}{6}$}
