...
public class LibraryTest {
	...
	public static void main(String[] args) {
		...
		//check out multiple books to same holder
		if (!lib.checkout(9781843190479L, "Ben Allred", 9, 1, 2016))
			System.err.println("TEST FAILED -- medium library: checkout");
		if (!lib.checkout(9781843192954L, "Ben Allred", 9, 1, 2016))
			System.err.println("TEST FAILED -- medium library: checkout");
		if (!lib.checkout(9781843190028L, "Ben Allred", 9, 1, 2016))
			System.err.println("TEST FAILED -- medium library: checkout");
		
		//makes sure books were checked out properly
		booksCheckedOut = lib.lookup("Ben Allred");
		if (booksCheckedOut == null || booksCheckedOut.size() != 3)
			System.err.println("TEST FAILED -- medium library: lookup(holder) INSTANTIATION ERROR");	
		for(int book = 0; book < booksCheckedOut.size(); book++)
		{
			if (!booksCheckedOut.get(book).equals(new Book(9781843190479L, "Anthony J D Burns", "Demogorgon Rising"))
					&& !booksCheckedOut.get(book).equals(new Book(9781843192954L, "Dennis Radha-Rose", "The Anxiety Relief Program"))
					&& !booksCheckedOut.get(book).equals(new Book(9781843190028L, "Moyra Caldecott", "Crystal Legends")))
				System.err.println("TEST FAILED -- medium library: lookup(holder) BOOKS NOT FOUND");
			
			if (!booksCheckedOut.get(book).getHolder().equals("Ben Allred"))
				System.err.println("TEST FAILED -- medium library: lookup(holder) HOLDER NOT CORRECT");
			
			if(!booksCheckedOut.get(book).getDueDate().equals(new GregorianCalendar(2016, 9, 1)))
				System.err.println("TEST FAILED -- medium library: lookup(holder) DUEDATE NOT CORRECT");				
		}
		if(!lib.checkin("Ben Allred"))
