...
/**
 * Class representation of a library (a collection of generic library books).
 */
public class LibraryGeneric<Type> {
	...
	public Type lookup(long isbn) {
		for (LibraryBookGeneric<Type> libBook : library) {
			if (libBook.getIsbn() == isbn) {
				return libBook.getHolder();
			}
		}
		return null;
	}
	...
	public boolean checkout(long isbn, Type holder, int month, int day, int year) {
		for (LibraryBookGeneric<Type> libBook : library) {
			if (libBook.getIsbn() == isbn) {
				if (libBook.getHolder() == null) {
					libBook.checkOut(holder, new GregorianCalendar(year, month, day));
					return true;
				}
			}
		}
