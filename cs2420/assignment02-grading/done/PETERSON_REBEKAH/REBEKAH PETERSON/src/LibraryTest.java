...
public class LibraryTest {
	...
	public static void main(String[] args) {
		...
		if (booksCheckedOut == null)
			System.err.println("one TEST FAILED -- small library: lookup(holder)");
		// two
		if (booksCheckedOut.size() != 1)
			System.err.println("two TEST FAILED -- small library: lookup(holder)");
		// three
		if (!booksCheckedOut.get(0).equals(new Book(9780330351690L, "Jon Krakauer", "Into the Wild")))
			System.err.println("three TEST FAILED -- small library: lookup(holder)");
		// four
		if (!booksCheckedOut.get(0).getHolder().equals("Jane Doe"))
			System.err.println("four TEST FAILED -- small library: lookup(holder)");
		// five
		if (!booksCheckedOut.get(0).getDueDate().equals(new GregorianCalendar(2008, 1, 1)))
			System.err.println("five TEST FAILED -- small library: lookup(holder)");
