...
public class LibraryTest {
	...
	public static void main(String[] args) {
		...
		if( lib.lookup(9781843190769L) != null ) {
			System.err.println("TEST FAILED -- medium library: lookup(isbn)");
		}
		if( !lib.checkout(9781843190769L, "John Doe", 6, 14, 1993) ) {
			System.err.println("TEST FAILED -- medium library: checkout");
		}
		lib.checkout(9781843190875L, "John Doe", 6, 11, 1993);
		
		ArrayList<LibraryBook> booksCheckedOut2 = lib.lookup("John Doe");
		
		if( booksCheckedOut2 == null || booksCheckedOut2.size() != 2 
				|| !booksCheckedOut2.get(1).equals(new Book(9781843190875L, "Renee Angers", "Ice and a Curious Man"))
				|| !booksCheckedOut2.get(1).getAuthor().equals("Renee Angers")
				|| !booksCheckedOut2.get(1).getHolder().equals("John Doe")
				|| !booksCheckedOut2.get(1).getDueDate().equals(new GregorianCalendar(1993, 6, 11))) {
		...
		ArrayList<LibraryBook> noBooksCheckedOut = lib.lookup("John Doe"); // Both books have been checked in already, so should return an empty ArrayList
		if( noBooksCheckedOut.size() != 0 ) {
			System.err.println("TEST FAILED -- lookup(holder) with no books");
		}
