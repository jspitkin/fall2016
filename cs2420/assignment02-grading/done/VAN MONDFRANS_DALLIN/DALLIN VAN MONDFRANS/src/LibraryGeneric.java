...
public class LibraryGeneric<Type> {
	...
	public Type lookup(long isbn) {
		...
		for (int index = 0; index < library.size(); index++) {
			if (isbn == library.get(index).getIsbn()) {
	...
	public ArrayList<LibraryBookGeneric<Type>> lookup(Type holder) {
		...
		for (int index = 0; index < library.size(); index++) {
			if (holder == library.get(index).getHolder()) {
				holderBookList.add(library.get(index));
			}
		}
		...
		for (int index = 0; index < library.size(); index++) {
			if (isbn == library.get(index).getIsbn()) {
				if (library.get(index).getHolder() == null) {
					...
					library.get(index).checkBookOut(holder, dueDate);
					return true;
				}
			}
		}
	 ...
	 *            -- holder of the library books to be checked in
	 */
	public boolean checkin(PhoneNumber holder) {
...
// Phase 3 Features --------------------------------------------------------------------------------
	   ...
	   * 
	   * @param month
	   * 		- numeric representations of months begin at 0 (January)
	   * @param day
	   * @param year 
	   */
	  public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day, int year) {
		  ...
		  for( int index = 0; index < library.size(); index++ ) {
			  if( library.get(index).getDueDate() != null) {
				  if( library.get(index).getDueDate().compareTo(date) < 0 ) {
	 ...
	 /**
	  ...
	  protected class OrderByAuthor implements Comparator<LibraryBookGeneric<Type>> {
		  ...
		  /**
		   * Returns a negative number if lhs author is lexicographically before rhs author, 
		   * a positive number if the rhs author is before the lhs author.
		   * If they both have the same author, then the same values are returned based upon
		  ...
		  public int compare(LibraryBookGeneric<Type> lhs, LibraryBookGeneric<Type> rhs) {
			  ...
			  if( lhsAuthor.toString() == rhsAuthor.toString() ) {
				  String lhsTitle = lhs.getTitle();
				  String rhsTitle = rhs.getTitle();
				  
				  return (int) lhsTitle.compareTo(rhsTitle);
				  
			  } else {
				  
				  return (int) lhsAuthor.compareTo(rhsAuthor);
	  ...
	  protected class OrderByDueDate implements Comparator<LibraryBookGeneric<Type>> {
	      ...
	      /**
	       * Returns a negative number if the lhs dueDate is before the rhs dueDate, a 
	       * positive number if the lhs dueDate is after the rhs dueDate, or a 0 if they 
	       * are equal.
	       */
		  public int compare(LibraryBookGeneric<Type> lhs, LibraryBookGeneric<Type> rhs) {
			  ...
			  return (int) lhsDate.compareTo(rhsDate);
