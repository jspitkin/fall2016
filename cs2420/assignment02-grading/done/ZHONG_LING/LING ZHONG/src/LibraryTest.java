...
public class LibraryTest {
	...
	public static void main(String[] args) {
		...
		lib.checkout(9781843190004L, "Steve", 6, 5, 2016);
		if (!("Steve" == lib.lookup(9781843190004L))) {
			System.err.println("TEST FAILED -- medium library: lookup (isbn)");
		}
		lib.checkin(9781843190004L);
		if (!(null == lib.lookup(9781843190004L))) {
			System.err.println("TEST FAILED -- medium library: lookup (isbn)");
		}
		lib.checkout(9781843190004L, "Steve", 6, 5, 2016);
		lib.checkout(9781843193319L, "Steve", 6, 5, 2016);
		lib.checkout(9781843192701L, "Steve", 6, 5, 2016);
		lib.checkin("Steve");
		if (!(null == lib.lookup(9781843190004L))) {
			System.err.println("TEST FAILED -- medium library: checkin (holder)");
		}
		if (!(null == lib.lookup(9781843193319L))) {
			System.err.println("TEST FAILED -- medium library: checkin (holder)");
		}
		if (!(null == lib.lookup(9781843192701L))) {
			System.err.println("TEST FAILED -- medium library: checkin (holder)");
		}
