...
public class LibraryGeneric<Type> {
	...
	public Type lookup(long isbn) {
		for (int index = 0; index < library.size() - 1; index++) {
			LibraryBookGeneric<Type> temp = library.get(index);
			if (isbn == temp.getIsbn()) {
				return temp.getHolder();
			}
		}
		return null;
	}
	...
	public ArrayList<LibraryBookGeneric<Type>> lookup(Type holder) {
		ArrayList<LibraryBookGeneric<Type>> booksOut = new ArrayList<LibraryBookGeneric<Type>>();
		for (int index = 0; index < library.size() - 1; index++) {
			LibraryBookGeneric<Type> temp = library.get(index);
			if (holder.equals(temp.getHolder())) {
				booksOut.add(temp);
			}
		}
		return booksOut;
		...
		for (int index = 0; index < library.size() - 1; index++) {
			if (library.get(index).getIsbn() == isbn) {
				if (library.get(index).getHolder() == null) {
					LibraryBookGeneric<Type> temp = library.get(index);
					temp.checkOut(holder, year, month, day);
	...
	public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day, int year) {
		GregorianCalendar tester = new GregorianCalendar(year, month, day);
		ArrayList<LibraryBookGeneric<Type>> libraryCopy = new ArrayList<LibraryBookGeneric<Type>>();
		for (int index = 0; index < library.size()-1; index++) {
			if (library.get(index).getDueDate().compareTo(tester) > 1) {
	...
	protected class OrderByDueDate implements Comparator<LibraryBookGeneric<Type>> {
		...
		public int compare(LibraryBookGeneric<Type> lhs, LibraryBookGeneric<Type> rhs) {
			int result = lhs.getDueDate().compareTo(rhs.getDueDate());	
			return result;
		}
	}
