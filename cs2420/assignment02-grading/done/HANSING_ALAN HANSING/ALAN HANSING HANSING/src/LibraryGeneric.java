...
public class LibraryGeneric<Type>  {
	...
	public ArrayList<LibraryBookGeneric<Type>> lookup(Type holder) {
		ArrayList<LibraryBookGeneric<Type>> toReturn = new ArrayList<LibraryBookGeneric<Type>>();
		for(int i = 0; i < library.size(); i++){
			if(!(library.get(i).getHolder() == null)){
				if(library.get(i).getHolder().equals(holder)){
					toReturn.add(library.get(i));
				}
			}
		}
		return toReturn;
	 ...
	 /**
	  ...
	  public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day,
		  ...
		  ArrayList<LibraryBookGeneric<Type>> libraryCopy = new ArrayList<LibraryBookGeneric<Type>>();
		    ...
		    for(int i = 0; i < library.size(); i++){
		    	if((library.get(i).getDueDate() != null)&&(currentDate.compareTo(library.get(i).getDueDate()) > 0)){
