...
public class LibraryGeneric<Type> {
	...
	public Type lookup(long isbn) {
		// check each book in the array, if ISBN is found, return the holder of that book
		...
		// no book with that ISBN was found, return null
		return null;
	}
	...
	public ArrayList<LibraryBookGeneric<Type>> lookup(Type holder) {
		// create a new list for storing just the books that are checked out by the holder
		ArrayList<LibraryBookGeneric<Type>> newList = new ArrayList<LibraryBookGeneric<Type>>();
		
		// search for any books associated with that holder, and add any found ones to the new array
		for(LibraryBookGeneric<Type> book : this.library) {
	...
	public boolean checkout(long isbn, Type holder, int month, int day, int year) {
		...
		// search for a book with the specified ISBN
		for(LibraryBookGeneric<Type> book : this.library) {
	        if(book.getIsbn() == isbn) {
	    		GregorianCalendar dueDate = new GregorianCalendar(year, month, day);
	    		// try to check the book out, if result is false, the book is already checked out
	...
	public boolean checkin(Type holder) {
		...
		// check in each book that was found to be associated with the holder
	...
	public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day, int year) {
		...
		// search for books with due date is older than the input date
		for(LibraryBookGeneric<Type> book : library) {
	        if(book.getDueDate() != null && book.getDueDate().before(new GregorianCalendar(year, month, day))) {
	...
	protected class OrderByAuthor implements Comparator<LibraryBookGeneric<Type>> {
		...
		/**
		 * Returns a negative value if lhs comes before rhs. Returns a
		 * positive value if lhs comes after rhs. Returns 0 if lhs and rhs
		 * are equal.
		 */
		public int compare(LibraryBookGeneric<Type> lhs, LibraryBookGeneric<Type> rhs) {
			int authorCompare = lhs.getAuthor().compareToIgnoreCase(rhs.getAuthor());
			if (authorCompare != 0) return authorCompare;
			
			//if authors are the same, return the title order
