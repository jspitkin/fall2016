...
public class Library {
	...
	public String lookup(long isbn) {
		for (int whichBook = 0; whichBook < library.size(); whichBook++) {
			if (library.get(whichBook).getIsbn() == isbn) {
				return library.get(whichBook).getHolder();
			}
		}
		return null;
	}
	...
	public ArrayList<LibraryBook> lookup(String holder) {
		ArrayList<LibraryBook> finalList = new ArrayList<LibraryBook>();
		for (int holderCount = 0; holderCount < library.size(); holderCount++) {
			if (library.get(holderCount).getHolder() == null) { // If it's null, it will continue and 
				continue;										// avoid a null pointer exception.
			}
			else {
	...
	public boolean checkout(long isbn, String holder, int month, int day, int year) {
		...
		for (int bookSearch = 0; bookSearch < library.size(); bookSearch++) {
			if (library.get(bookSearch).getDueDate() == null && library.get(bookSearch).getIsbn() == isbn) {
				// ^ If the dueDate is null and it has the same ISBN, then the book is available and 
				// is the correct book. 
	...
	public boolean checkin(long isbn) {
		for (LibraryBook book : library) {
			if (book.getIsbn() == isbn && book.getHolder() != null) {
				// If if the holder isn't null and has the same ISBN, the book is checked out 
				// and qualifies to be checked in. 
				book.checkIn();
				return true;
			}
		}
