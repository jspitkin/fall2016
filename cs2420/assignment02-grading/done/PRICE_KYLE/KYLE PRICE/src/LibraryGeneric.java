...
public class LibraryGeneric<Type> {
	...
	public LibraryGeneric() {
	  ...
	  public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day, int year) {
		  ...
		  for (LibraryBookGeneric<Type> book : library) {
			  ...
			  if (book.getDueDate().compareTo(currentDate) <= 0) { // filling the new array with overdue books
	   ...
	   */
	  private static <Type> void sort(ArrayList<Type> list,
	      Comparator<Type> c) {
	...
	protected class OrderByDueDate implements Comparator<LibraryBookGeneric<Type>> { 
		public int compare(LibraryBookGeneric<Type> lhs, LibraryBookGeneric<Type> rhs) {
			if (lhs.getDueDate().compareTo(lhs.getDueDate()) > 0) { 
				return 1;
			}
			if (lhs.getDueDate().compareTo(lhs.getDueDate()) < 0) {
				return -1;
	...
	public String lookup(long isbn) {
		for (LibraryBookGeneric<Type> book : library) {
			if (book.getIsbn() == isbn) {
				String finalString = (String) book.getHolder();
				return finalString;
			}
		}
	...
	public ArrayList<LibraryBookGeneric<Type>> lookup(Type holder) {
		ArrayList<LibraryBookGeneric<Type>> finalList = new ArrayList<LibraryBookGeneric<Type>>();
		for (int holderCount = 0; holderCount < library.size(); holderCount++) {
			if (library.get(holderCount).getHolder() == null) {
				continue;
			}
			else {
				finalList.add(library.get(holderCount));
			}
		}
		return finalList;
	}
	...
	public boolean checkout(long isbn, Type holder, int month, int day, int year) {
		...
		for (int bookSearch = 0; bookSearch < library.size(); bookSearch++) {
			if (library.get(bookSearch).getDueDate() == null && library.get(bookSearch).getIsbn() == isbn) {
				library.get(bookSearch).checkOut(holder, dueDate);
				return true;
			}
		}
		return false;
