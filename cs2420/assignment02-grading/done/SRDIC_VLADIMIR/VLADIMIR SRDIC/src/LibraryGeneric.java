...
public class LibraryGeneric<Type> {
	...
	protected class OrderByAuthor implements Comparator<LibraryBookGeneric<Type>> {
		/**
		 * Returns -1 if the lhs book's author is before the rhs book's
		 * author alphabetically. Returns 0 if they are the same.
		 * Returns 1 if the lhs author is after the rhs author.
		 */
		public int compare(LibraryBookGeneric<Type> lhs, LibraryBookGeneric<Type> rhs) {
	...
	protected class OrderByDueDate implements Comparator<LibraryBookGeneric<Type>> {
		/**
		 * Returns a negative value if the due date of the lhs is smaller 
		 * than the due date of the rhs. Returns a positive value if the 
		 * due date of the lhs is larger than the due date of the rhs. 
	...
	public Type lookup(long isbn) {
		for (LibraryBookGeneric<Type> curr : library) {
			if (curr.getIsbn() == isbn) {
				return curr.getHolder();
			}
		}
		return null;
	}
	...
	public ArrayList<LibraryBookGeneric<Type>> lookup(Type holder) {
		...
		for (LibraryBookGeneric<Type> curr : library) {
			if (curr.getHolder() != null && curr.getHolder().equals(holder)) {
				heldBooks.add(curr);
			}
		}
	...
	public boolean checkout(long isbn, Type holder, int month, int day, int year) {
		for (LibraryBookGeneric<Type> curr : library) {
			...
			if (curr.getIsbn() == isbn) {
				if (curr.getCheckedOut()) {
					return false;
				} else {
					curr.checkOut(holder, new GregorianCalendar(year, month, day));
					return true;
				}
			}
		}
	...
	public boolean checkin(Type holder) {
		ArrayList<LibraryBookGeneric<Type>> booksOut = lookup(holder);
		for (LibraryBookGeneric<Type> b : booksOut) {
			b.checkIn();
		}
