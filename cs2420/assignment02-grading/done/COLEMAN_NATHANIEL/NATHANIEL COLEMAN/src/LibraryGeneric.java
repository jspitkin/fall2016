...
/**
 * Generic class for a library, a collection of generic library books.
 * 
...
public class LibraryGeneric<Type> {
	...
	public boolean checkin(Type holder) {
		ArrayList<LibraryBookGeneric<Type>> list = this.lookup(holder);
		
		if (list.size() == 0)
			return false;
		
		for (LibraryBookGeneric<Type> book : list)
		{
			book.setHolder(null);
			book.setDueDate(null);
		}
	  ...
	  public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day, int year) {
		  ...
		  for (LibraryBookGeneric<Type> book : library) {			//Copies only overdue books into 
			  if (book.getDueDate().compareTo(inputDate) > 0) {		//	overdueList.
	 ...
	 /**
	  ...
	  protected class OrderByAuthor implements Comparator<LibraryBookGeneric<Type>> {
		  ...
		  @Override
		  public int compare(LibraryBookGeneric<Type> lhs, LibraryBookGeneric<Type> rhs) {
				int authorCompareResult = lhs.getAuthor().compareTo(rhs.getAuthor());
				
				if (authorCompareResult == 0)
				{
					authorCompareResult = lhs.getTitle().compareTo(rhs.getTitle()); 
				}
