	...
	public class LibraryGeneric<Type> {
		...
		public Type lookup(long isbn) {
			  for (int x = 0; x < library.size(); x++)
				  if (library.get(x).getIsbn() == isbn)
				  {
					  return library.get(x).getHolder();
				  }
			  return null;
		  }
		...
		public ArrayList<LibraryBookGeneric<Type>> lookup(Type holder) {
			...
			for(int x=0; x<library.size(); x++){
				if(holder.equals(library.get(x).getHolder())){
					list.add(library.get(x));
				}
			}
			return list;
		}
		...
		public boolean checkout(long isbn, Type holder, int month, int day, int year) {
			for(int x=0; x<library.size(); x++ ){
				if(library.get(x).getIsbn()==isbn){
					library.get(x).setHolder(holder);
					library.get(x).setDateDue(new GregorianCalendar(year,month,day));
					return true;
				}
			}
				return false;
		...
		public boolean checkin(long isbn) {
			for(int x=0; x<library.size();x++){
				if(library.get(x).getIsbn()==isbn){
					if(library.get(x).getHolder()!=null){
						library.get(x).setHolder(null);
						library.get(x).setDateDue(null);
						return true;
					}
					return true;
				}
		 ...
		 /**
		  ...
		  public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day,int year) {
		    GregorianCalendar date=new GregorianCalendar();
			  ArrayList<LibraryBookGeneric<Type>> sorted = new ArrayList<LibraryBookGeneric<Type>>();
			  for(int x=0;x<library.size();x++){
				  if(library.get(x).getDueDate()!=null&&library.get(x).getDueDate().before(date)){
					  sorted.add(library.get(x));
				  }
			  }
