...
public class LibraryGeneric<Type> {
	...
	public Type lookup(long isbn) {
		for(LibraryBookGeneric<Type> selected : library) {
			if(selected.getIsbn() == isbn) {
				return selected.getHolder();
			}
		}
	...
	public ArrayList<LibraryBookGeneric<Type>> lookup(Type holder) {
		...
		for(LibraryBookGeneric<Type> selected : library) {
			if(selected.getHolder() == holder)
				holderLibrary.add(selected);
		}
	...
	public boolean checkout(long isbn, Type holder, int month, int day, int year) {
		for(LibraryBookGeneric<Type> selected : library) {
			if((lookup(isbn) != null) && (selected.getIsbn() == isbn)) {
				selected.checkout(holder, new GregorianCalendar(year, month, day));
				return true;
			}
		}
		
		// base case - if no book with specified ISBN is in the library
		return false;
	}
	...
	public boolean checkin(long isbn) {
		...
		// base case(s) - no book with ISBN in library 
		//              - or book with ISBN is already checked in
		return false;
	}
	...
	public boolean checkin(Type holder) {
		ArrayList<LibraryBookGeneric<Type>> specifiedBooks = lookup(holder);
		if(specifiedBooks.size() > 0) {
			for(LibraryBookGeneric<Type> book : specifiedBooks) {
				book.checkin();
			}
			return true;
		}
		// base case - no book in library matches holder
		return false;
	}
	  ...
	  public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day,
	      int year) {
		// if statement check to see if book.getDueDate != null && book.getDueDate.before(new GregCal(perameters get overdue list))  
	    // FILL IN -- do not return null
		    ArrayList<LibraryBookGeneric<Type>> libraryCopy = new ArrayList<LibraryBookGeneric<Type>>();
		    libraryCopy.addAll(library);
		    ...
		    for(LibraryBookGeneric<Type> book : newLibrary) {
		    	if(book.getDueDate() != null && book.getDueDate().before(book)) {
		    		newLibrary.add(0, book);
		    	}
		    }
	...
	Comparator<LibraryBookGeneric<Type>> {
		...
		@Override
		public int compare(LibraryBookGeneric<Type> arg0, LibraryBookGeneric<Type> arg1) {
