...
public class TAHashTableGrader {
	...
	public static class HashFunctorTester implements Runnable {
		...
		public void run() {
			HashFunctor hf;
			try {
				//NOTE: The assignment did not specify the constructor signature for these, so if
	...
	public static class HashTableTester implements Runnable {
		...
		public boolean doTest() {
			...
			case 22:
				...
				contents = new ArrayList<String>(Arrays.asList("foo", "bar", "baz", "cat",
																"fez", "act", "", "a"));
				table.addAll(contents);
				contents.add("hey");
				return !table.containsAll(contents);
	...
	public static void main(String[] args) {
	    ...
	    // TA's should still spot-check that the functors are always consistent, would work for
	    ...
	    System.out.println("QuadProbeHashTable:\t\t\t\t"
	    					+ Math.round(tablePoints[QUAD_PROBE] * 15) + "/15");
	    System.out.println("SeparateChainingHashTable:\t\t\t"
	    					+ Math.round(tablePoints[SEP_CHAIN] * 15) + "/15");
