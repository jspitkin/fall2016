...
/**
 * This class manages matricies, providing the methods equals, toString, times, and plus.
 * @author Prathusha, CS 2420
 * Fall 2016, CS 2420, Miriam Meyer
 */
public class Matrix {
	int numRows;
	int numColumns;
	...
	public Matrix times(Matrix matrix)
	{
		...
		result = new Matrix(new int[this.numRows][matrix.numColumns]);
		for(int i=0; i<=this.numRows-1; i++){
			for(int j=0; j<=matrix.numColumns-1; j++){
				for(int k=0; k<=this.numColumns-1; k++){
	...
	public Matrix plus(Matrix matrix)
	{
		 ...
		 */
		if(this.numRows==matrix.numRows && this.numColumns==matrix.numColumns){
			result = new Matrix(new int[this.numRows][this.numColumns]);
			for(int i=0; i<=this.numRows-1; i++){
				for(int j=0; j<=this.numColumns-1; j++){
