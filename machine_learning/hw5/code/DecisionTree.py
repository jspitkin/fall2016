...
class DecisionTree:
            ...
            with open(path) as file:
                for line in file:
                    ...
                    for feature in line[:-1]:
                        example.add_feature(feature)
                    if len(line) >= 1:
                        example.update_label(line[-1])
                    examples.append(example)

        if self.replace_method == 1:
            examples = self.replace_method_one(examples)
        elif self.replace_method == 2:
            examples = self.replace_method_two(examples)
        return examples
    ...
    def replace_method_one(self, data):
        for example in data:
            for feature in example.features:
                if feature == "?":
                    most_common_label = Counter(feature).most_common()[0]
                    feature = most_common_label
        return data

    def replace_method_two(self, data):
        for example in data:
            for feature in example.features:
                if feature == "?":
                    most_common_label = Counter(feature).most_common()[0]
